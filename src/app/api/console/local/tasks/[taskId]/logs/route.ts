...
export const GET = apiHandler(async (
  ...
  const url = new URL(`${normalizeBaseUrl(baseUrl)}/api/integrations/waoowaoo/v1/tasks/${encodeURIComponent(taskId)}/logs`)
  ...
  if (!res.ok) {
    throw new ApiError('INTERNAL_ERROR', { message: `local task logs request failed: ${res.status} ${text.slice(0, 200)}` })
  }
