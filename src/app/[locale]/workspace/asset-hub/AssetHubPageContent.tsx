...
export function AssetHubPageContent({ mobile = false, onBack }: { mobile?: boolean; onBack?: () => void }) {
    ...
    return (
        ...
        >
            <div className={mobile ? 'min-h-screen bg-[#f4f6f8]' : 'glass-page min-h-screen'}>
                {!mobile ? <Navbar /> : null}
                <QueueProgressPopup />
            <div className={mobile ? 'mx-auto max-w-md px-4 pb-24 pt-4' : 'max-w-7xl mx-auto px-4 py-6'}>
                {/* 页面标题 */}
                <div className={mobile ? 'sticky top-0 z-30 mb-4 rounded-[22px] border border-slate-200 bg-white/95 p-3 shadow-sm backdrop-blur' : 'mb-6'}>
                    ...
                    <h1 className={`${mobile ? 'text-base' : 'text-2xl'} font-bold text-[var(--glass-text-primary)]`}>{t('title')}</h1>
